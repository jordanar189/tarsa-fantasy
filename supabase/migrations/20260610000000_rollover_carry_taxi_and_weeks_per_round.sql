-- Carry the newer league/team fields through dynasty rollover.
--
-- rollover_league inserts the child league + cloned teams with explicit column
-- lists that predate two fields, so rollover silently lost them:
--   * leagues.weeks_per_round  → child reverted to 1-week playoff rounds
--   * teams.taxi               → every team's taxi stash was wiped (data loss)
--
-- (Taxi *config* — slot count + max experience — already carries over because
-- it lives inside leagues.roster_config JSONB, which is copied wholesale.)
--
-- Same signature as the previous definition, so CREATE OR REPLACE swaps it in
-- place. Runs after the taxi (teams.taxi) and weeks_per_round migrations, so
-- both columns exist here.
